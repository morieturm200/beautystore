...
/**
 * 1. ЗАПИТ ДО БАЗИ
 * Вибираємо товари, які:
 * - Беруть участь у розіграші (is_giveaway_participant)
 * - АБО мають заповнений promo_type (1+1=3, GIFT тощо)
 * - АБО мають акційну ціну (is_sale)
 */
...
// Якщо товарів немає, спробуємо вивести просто останні 5 товарів, щоб блок не був порожнім
if ($res_brands->num_rows == 0) {
    $res_brands = $conn->query("SELECT p.*, i.image_url FROM product p LEFT JOIN Images i ON p.product_id = i.product_id AND i.is_primary = 1 LIMIT 5");
}
