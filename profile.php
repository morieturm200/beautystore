<?php
/**
 * PRIVÉ MASTER CABINET v9.7 - CONTRAST LUXURY EDITION
 * --------------------------------------------------
 * ДИЗАЙН: Світлий глянець з глибокими чорними акцентами
 * БОТ: Золотий (Gold) з чорною коронкою - МАКСИМАЛЬНА ВИДИМІСТЬ
 * ЛОГІКА: Повне збереження структури та функціоналу
 */

session_start();

// Детальний вивід помилок для розробника
error_reporting(E_ALL);
ini_set('display_errors', 1);

/**
 * 1. ВЕРИФІКАЦІЯ ДОСТУПУ
 * Перевірка сесії клієнта перед завантаженням інтерфейсу
 */
...
try {
    ...
    /**
     * 3. ОБРОБКА POST-ЗАПИТІВ (БІЗНЕС-ЛОГІКА)
     */

    // Логіка скасування замовлення клієнтом
    ...
    // Логіка оновлення персональних даних (Master Synchronization)
    if (isset($_POST['update_profile'])) {
        ...
        // Використання оператора Null Coalescing для безпечного оновлення
        $stmt->execute([
            $_POST['first_name'] ?? '', 
            $_POST['last_name'] ?? '', 
            $_POST['phone_number'] ?? '', 
    ...
    // Отримання повного об'єкта користувача
    $stmt = $pdo->prepare("SELECT * FROM customer WHERE customer_id = ?");
    $stmt->execute([$current_user_id]);
    $user_data = $stmt->fetch();

    // Отримання історії замовлень з агрегацією товарів
    $order_sql = "
        SELECT o.*,
    ...
    // Калькуляція фінансової статистики для VIP-статусу
    $total_spent = 0;
    ...
    // НОВЕ: Отримання кількості товарів у кошику та списку бажань
    ...
    // Отримання повної історії листування з таблиці Support
