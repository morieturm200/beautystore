...
// 2. ОТРИМАННЯ ДАНИХ ДЛЯ ВИПАДАЮЧИХ СПИСКІВ (SELECT)
$categories_list = [];
$customers_list = [];

try {
    // Список категорій для запиту №2
...
// 3. ОБРОБКА POST-ЗАПИТІВ (БЕЗ ЗМІНИ URL)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action) {
    ...
    try {
        switch ($action) {
            ...
            // -------------------------------------------------------------------------
            // 1. ЛІСТИНГ 2.4: ПОВНА ІНФОРМАЦІЯ ПРО КОЖЕН ПРОДУКТ
            // -------------------------------------------------------------------------
            case 'q1_full_info':
                $sql = "SELECT 
                            p.product_id AS 'ID', 
            ...
            // -------------------------------------------------------------------------
            // 2. ЛІСТИНГ 2.5: УСІ ПРОДУКТИ З ОБРАНОЇ КАТЕГОРІЇ (ДИНАМІЧНО)
            // -------------------------------------------------------------------------
            case 'q2_by_category':
            ...
            // -------------------------------------------------------------------------
            // 3. ЛІСТИНГ 2.6: ТОП 5 ПРОДАНИХ ПРОДУКТІВ
            // -------------------------------------------------------------------------
            case 'q3_top5_sold':
                $sql = "SELECT 
                            p.product_id AS 'ID', 
            ...
            // -------------------------------------------------------------------------
            // 4. ЛІСТИНГ 2.7: ПРОДУКТИ, КУПЛЕНІ ОБРАНИМ КЛІЄНТОМ (ДИНАМІЧНО)
            // -------------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------------
            // 5. ЛІСТИНГ 2.8: ТОП КАТЕГОРІЇ ЗА ПРОДАЖАМИ
            // -------------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------------
            // 6. ЛІСТИНГ 2.9: КЛІЄНТИ З КІЛЬКІСТЮ ЗАМОВЛЕНЬ > 1
            // -------------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------------
            // 7. ЛІСТИНГ 2.10: НАЙКРАЩЕ ПРОДАВАНИЙ ПРОДУКТ ВІД КОЖНОГО ВИРОБНИКА
            // -------------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------------
            // 8. ЛІСТИНГ 2.11: КЛІЄНТИ, ЯКІ ВИТРАТИЛИ ПОНАД СУМУ (ДИНАМІЧНО)
            // -------------------------------------------------------------------------
            ...
            // -------------------------------------------------------------------------
            // 9. ЛІСТИНГ 2.12: ПРОДУКТИ, ЯКІ НІКОЛИ НЕ КУПУВАЛИСЬ
            // -------------------------------------------------------------------------
            case 'q9_never_sold':
                $sql = "SELECT 
                            p.product_id AS 'ID', 
            ...
            // -------------------------------------------------------------------------
            // 10. ЛІСТИНГ 2.13: ПРОДУКТИ З ЗАПАСОМ МЕНШЕ 15 ОДИНИЦЬ
            // -------------------------------------------------------------------------
            case 'q10_low_stock':
                $sql = "SELECT 
                            product_id AS 'ID', 
...
// 4. ФУНКЦІЯ ГЕНЕРАЦІЇ ТАБЛИЦІ РЕЗУЛЬТАТІВ
...
<body class="p-6 md:p-12">
    ...
    <main class="max-w-7xl mx-auto">
        ...
        <details <?= $action === 'q1_full_info' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.4</span> Повна інформація про кожен продукт</summary>
        ...
        <details <?= $action === 'q2_by_category' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.5</span> Усі продукти з обраної категорії</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q3_top5_sold' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.6</span> Топ 5 проданих продуктів</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q4_client_history' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.7</span> Усі продукти, куплені обраним клієнтом</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q5_top_categories' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.8</span> Топ категорії за продажами</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q6_loyal_clients' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.9</span> Клієнти з кількістю замовлень більше 1</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q7_brand_flagships' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.10</span> Найкраще продаваний продукт від кожного виробника</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q8_high_spenders' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.11</span> Клієнти, які витратили понад вказану суму</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q9_never_sold' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.12</span> Продукти, які ніколи не купувались</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
        ...
        <details <?= $action === 'q10_low_stock' ? 'open' : '' ?>>
            <summary><span class="text-[#c5a059] mr-3">2.13</span> Продукти з запасом менше 15 одиниць</summary>
            <div class="details-body">
                <div class="mb-8 p-6 bg-[#fcfaf8] rounded-2xl border border-[#f0e6e0] text-xs font-bold text-gray-600 italic">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Плавний скрол до відкритого елемента після відправки форми
            ...
            // Робота як справжній акордеон (закриваємо інші, коли відкриваємо один)
            const detailsElements = document.querySelectorAll('details');
            detailsElements.forEach((targetDetail) => {
                targetDetail.addEventListener('click', (e) => {
                    // Якщо клік був по формі або таблиці всередині, не закриваємо
