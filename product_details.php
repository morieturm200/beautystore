...
// 1. ЗАПИТ ТОВАРУ (з основною картинкою)
...
// 2. ОТРИМАННЯ ВСІХ КАРТИНКИ ДЛЯ ГАЛЕРЕЇ
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Montserrat', sans-serif; }
        ...
        .image-viewport:hover img { transform: scale(1.05); }
        ...
        /* СТИЛІ ДЛЯ СХОЖИХ ТОВАРІВ */
        .related-section { padding: 80px 0; border-top: 1px solid var(--border); text-align: center; }
        .related-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 30px; margin-top: 40px; }
        .related-card { text-decoration: none; color: inherit; transition: 0.3s; }
        .related-card:hover { transform: translateY(-10px); }
        .related-img { width: 100%; height: 300px; object-fit: cover; background: #f9f9f9; margin-bottom: 15px; }
        .related-name { font-size: 14px; font-weight: 600; margin-bottom: 5px; display: block; }
        .related-price { color: var(--accent); font-weight: 700; }
...
<div class="shop-container">
    <div class="product-hero">
        <div class="gallery-wrapper">
            <div class="thumbnails">
                <?php foreach($gallery_images as $index => $img_url): ?>
                    <img src="<?php echo $img_url; ?>" class="thumb-item <?php echo $index === 0 ? 'active' : ''; ?>" onclick="changePhoto(this.src, this)" onerror="this.src='https://via.placeholder.com/200x260'">
                <?php endforeach; ?>
            </div>
            <div class="image-viewport">
                ...
                <img id="main-photo" src="<?php echo $gallery_images[0]; ?>" alt="<?php echo htmlspecialchars($product['name']); ?>" onerror="this.src='https://via.placeholder.com/600x800'">
            </div>
        </div>
        <div class="purchase-column">
            ...
            <div style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 30px;">
                <p style="font-size: 12px; color: #888; margin-bottom:10px;"><i class="fas fa-certificate"></i> 100% Оригінальний продукт</p>
                <p style="font-size: 12px; color: #888;"><i class="fas fa-truck"></i> Безкоштовна доставка Privé</p>
            </div>
        </div>
    </div>
    ...
    <section class="related-section">
        <h2 style="font-family: 'Playfair Display', serif; font-size: 2.2rem; font-style: italic;">Також шукають</h2>
        <div class="related-grid">
            <?php while($rel = $related_res->fetch_assoc()): ?>
                <a href="product_details.php?id=<?php echo $rel['product_id']; ?>" class="related-card">
                    <img src="<?php echo $rel['img'] ?: 'img/products/'.$rel['product_id'].'.jpg'; ?>" class="related-img" onerror="this.src='https://via.placeholder.com/300x400'">
                    <span class="related-name"><?php echo htmlspecialchars($rel['name']); ?></span>
                    <span class="related-price">₴<?php echo number_format($rel['price'], 0, '.', ' '); ?></span>
                </a>
            <?php endwhile; ?>
        </div>
    </section>
...
<script>
    ...
    function toggleWishlist(btn, id) {
        ...
        .then(data => {
            if (data.status === 'error') { window.location.href = 'login_register.php?msg=login_required'; return; }
            ...
            const wishCount = document.querySelector('nav a[href*="wishlist"], .wish-count');
            if(wishCount) wishCount.innerHTML = `ОБРАНЕ (<span>${data.count}</span>)`;
