...
// 3. ЗБЕРЕЖЕННЯ ВІДПОВІДІ (Оновлення колонки admin_reply)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_reply'])) {
    $rev_id = intval($_POST['review_id']);
    $reply_text = trim($_POST['admin_reply']);
    $p_id = intval($_POST['p_id']);

    $stmt = $pdo->prepare("UPDATE Reviews SET admin_reply = ? WHERE review_id = ?");
    $stmt->execute([$reply_text, $rev_id]);
    
    header("Location: admin_prive.php?tab=reviews&product_id=$p_id&msg=Відповідь збережена"); 
    exit();
}

// 4. СПИСОК ТОВАРІВ (Ліва панель)
            ...
            <?php foreach($review_groups as $group): ?>
                <a href="?tab=reviews&product_id=<?= $group['product_id'] ?>" 
                   class="flex items-center gap-4 p-5 border-b border-gray-50 hover:bg-[#fcfaf8] transition relative <?= $active_product == $group['product_id'] ? 'bg-[#fdfaf9] border-r-8 border-[#d4a373]' : '' ?>">
                    ...
                    <img src="<?= $group['p_img'] ?? 'placeholder.jpg' ?>" class="w-14 h-14 rounded-2xl object-cover bg-gray-50 <?= $group['unreplied_count'] > 0 ? 'ring-2 ring-[#d4a373]' : '' ?>">
    ...
    <div class="flex-1 space-y-6">
        <?php if($active_product): ?>
            ...
            <?php foreach($product_reviews as $r): ?>
                <div class="bg-white p-8 rounded-[45px] shadow-sm border <?= empty($r['admin_reply']) ? 'border-[#d4a373] border-2' : 'border-[#f0e6e0]' ?> transition hover:shadow-md relative mb-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center">
                            <button onclick="document.getElementById('form-<?= $r['review_id'] ?>').classList.toggle('hidden')" 
                                ...
                                <?= !empty($r['admin_reply']) ? '✎ Редагувати відповідь' : '↵ Надати відповідь' ?>
                            </button>
                        </div>
