...
// --- НОВИЙ БЛОК СИНХРОНІЗАЦІЇ ДЛЯ ВИПРАВЛЕННЯ ВІДОБРАЖЕННЯ ---
if (isset($_SESSION['customer_id'])) {
    $c_id = intval($_SESSION['customer_id']);
    // Отримуємо актуальний стан кошика з бази
    $res_sync = $conn->query("SELECT product_id, quantity FROM Cart WHERE customer_id = $c_id");
    
    // Оновлюємо сесію даними з бази, щоб нижче по коду все відобразилось
...
// --- ЛОГІКА ОТРИМАННЯ ДАНИХ З БАЗИ АБО СЕСІЇ (ОНОВЛЕНО ДЛЯ ЗОБРАЖЕНЬ) ---
if (isset($_SESSION['customer_id'])) {
    $customer_id = intval($_SESSION['customer_id']);
    // Беремо дані з таблиці Cart, приєднуючи таблицю Product та Images
    ...
    // Якщо гість — використовуємо логіку з сесії + додаємо JOIN для зображень
